...
public class PlayerController : MonoBehaviour
{
    ...
    // initialization of player state
    private void Awake()
    {
    ...
    private void HandleMovementInput()
    {
        ...
        if(gameObject.name == "Yin")
        {
            ...
            if (Mathf.Abs(horizontalMovementInput) > 0 ||
                Mathf.Abs(verticalMovementInput) > 0
                && state != PlayerState.Dead)
            {
                movementForce.x = movementSpeed * horizontalMovementInput;
                movementForce.y = movementSpeed * verticalMovementInput;
            }
            MovePlayer(movementForce);
        }
    ...
    private void MovePlayer(Vector2 movementForce)
    {
        ...
        {
            float horizontalVelocity = movementForce.x;
            float verticalVelocity = movementForce.y;
            playerRigidbody.velocity = new Vector2(horizontalVelocity, verticalVelocity);
        }
    ...
    {
        // only yang can jump
        if (jumpInput && gameObject.name == "Yang")
        {
    ...
    private void DetermineState()
    {
        ...
        if(gameObject.name == "Yin")
        {
            StartCoroutine(ChangeStateToIdel());
        }

    }

    IEnumerator ChangeStateToIdel()
    {
